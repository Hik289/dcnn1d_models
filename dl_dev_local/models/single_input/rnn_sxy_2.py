...
class rnn_sxy_2_1(BasicModule):
    def __init__(self,input_dim=482,hidden_size = 30, num_layers = 1, dropout = 0):
        ...
        self.rnn1 = nn.LSTM(input_size = 12, hidden_size = 256)
    ...
    def forward(self,x):
        ...
        self.model_out['y_pred'] = x_signal[-1,:,-1]
        self.model_out['signals'] = x_signal[-1,:,-1]
...
class rnn_sxy_2_2(BasicModule):
    def __init__(self,input_dim=12):
        ...
        self.rnn1 = nn.LSTM(input_size = input_dim, hidden_size = 12,num_layers = 2)

        self.dropout = nn.Dropout(0.1)
        self.end_layer = nn.Linear(12,1)
        self.act = nn.ReLU()


    def forward(self,x):
        x = x.permute(1,0,2)[-10:,:,:]
        x_signal,_ = self.rnn1(x)
...
class rnn_sxy_2_3(BasicModule):
    ...
    def forward(self,x):
        ...
        x_signal,_ = self.rnn1(x.permute(1,0,2)[:,:,0].unsqueeze(-1))
...
class rnn_sxy_2_5(BasicModule):
    def __init__(self,input_dim=482,drop_rate=0.5):
        ...
        self.rnn1 = nn.LSTM(input_size = 12, hidden_size = 250, num_layers = 1)    

        self.rnn5 = nn.LSTM(input_size = 12, hidden_size = 250, num_layers = 1)
...
class rnn_sxy_2_6(BasicModule):
    def __init__(self,input_dim=482,drop_rate=0.5):
        ...
        self.rnn1 = nn.LSTM(input_size = 12, hidden_size = 250, num_layers = 3)    

        self.rnn5 = nn.LSTM(input_size = 12, hidden_size = 250, num_layers = 3)
...
class rnn_sxy_2_7(BasicModule):
    def __init__(self,input_dim=482,drop_rate=0.5):
        ...
        self.rnn1 = nn.LSTM(input_size = 12, hidden_size = 256, num_layers = 3)    

        self.rnn5 = nn.LSTM(input_size = 12, hidden_size = 256, num_layers = 3)

        self.pointwise1 = nn.Conv1d(in_channels=10, out_channels=1, kernel_size=1)
        self.pointwise2 = nn.Conv1d(in_channels=4, out_channels=1, kernel_size=1)

        self.dropout = nn.Dropout(0.1)     
        self.linear1 = nn.Linear(in_features= 256, out_features= 20)
        self.linear2 = nn.Linear(in_features= 20, out_features= 20)
        self.end_layer = nn.Linear(20,1)
        self.act = nn.ReLU()
        # for p in self.rnn.parameters():
...
class rnn_sxy_2_8(BasicModule):
    def __init__(self,input_dim=482,drop_rate=0.5):
        ...
        self.rnn1 = nn.LSTM(input_size = 12, hidden_size = 128, num_layers = 4)    

        self.rnn5 = nn.LSTM(input_size = 12, hidden_size = 128, num_layers = 2)
...
class rnn_sxy_2_9(BasicModule):
    def __init__(self,input_dim=482,drop_rate=0.5):
        ...
        self.rnn1 = nn.LSTM(input_size = 12, hidden_size = 128, num_layers = 3)    

        self.rnn5 = nn.LSTM(input_size = 12, hidden_size = 128, num_layers = 1)
...
class rnn_sxy_2_10(BasicModule):
    def __init__(self,input_dim=482,drop_rate=0.5):
        ...
        self.rnn1 = nn.LSTM(input_size = 12, hidden_size = 256)
        self.rnn2 = nn.LSTM(input_size = 256, hidden_size = 200)
        self.rnn3 = nn.LSTM(input_size = 200, hidden_size = 150)
        self.rnn4 = nn.LSTM(input_size = 150, hidden_size = 100)       

        self.rnn5 = nn.LSTM(input_size = 12, hidden_size = 256)
        self.rnn6 = nn.LSTM(input_size = 256, hidden_size = 150)
...
class rnn_sxy_2_12(BasicModule):
    def __init__(self,input_dim=482,drop_rate=0.5):
        ...
        self.rnn1 = nn.LSTM(input_size = 12, hidden_size = 256)
        self.rnn2 = nn.LSTM(input_size = 256, hidden_size = 10)

        self.rnn3 = nn.LSTM(input_size = 12, hidden_size = 20)
...
class rnn_sxy_2_13(BasicModule):
    def __init__(self,input_dim=482,drop_rate=0.5):
        ...
        self.rnn6 = nn.LSTM(input_size = 12, hidden_size = 200)
...
class rnn_sxy_2_7_false(BasicModule):
    def __init__(self,input_dim=482,drop_rate=0.5):
        super(rnn_sxy_2_7_false,self).__init__()
        self.model_name = 'rnn_sxy_2_7_false:daily'
        ...
        self.rnn6 = nn.LSTM(input_size = 12, hidden_size = 256, num_layers = 3)

        self.pointwise1 = nn.Conv1d(in_channels=10, out_channels=1, kernel_size=1)
        self.pointwise2 = nn.Conv1d(in_channels=4, out_channels=1, kernel_size=1)

        self.pointwise3 = nn.Conv1d(in_channels=2, out_channels=1, kernel_size=1)
    ...
    def forward(self,x):
        ...
        x_week = x.reshape((x.shape[0], 4, x.shape[1]//4, x.shape[-1]))
        x_week = torch.mean(x_week, axis = 2)

        x_week,_ = self.rnn6(x_week.permute(1,0,2))

        x_signal = self.pointwise1(x_signal[-10:,:,:].permute(1,0,2)).squeeze(1)
        x_day = self.pointwise2(x_day[-4:,:,:].permute(1,0,2)).squeeze(1)
        x_week = self.pointwise3(x_week[-2:,:,:].permute(1,0,2)).squeeze(1)

        x_signal = x_signal + x_day +x_week
        x_signal = self.dropout(x_signal)
        x_signal = self.act(x_signal)
        x_signal = self.linear1(x_signal)
...
class rnn_sxy_2_7_false_2(BasicModule):
    def __init__(self,input_dim=482,drop_rate=0.5):
        super(rnn_sxy_2_7_false_2,self).__init__()
        self.model_name = 'rnn_sxy_2_7_false_2:all_time'
        ...
        self.rnn7 = nn.LSTM(input_size = 12, hidden_size = 256, num_layers = 3)

        self.pointwise1 = nn.Conv1d(in_channels=10, out_channels=1, kernel_size=1)
        self.pointwise2 = nn.Conv1d(in_channels=4, out_channels=1, kernel_size=1)

        self.pointwise3 = nn.Conv1d(in_channels=2, out_channels=1, kernel_size=1)
        self.pointwise4 = nn.Conv1d(in_channels=1, out_channels=1, kernel_size=1)
    ...
    def forward(self,x):
        ...
        x_month = x.reshape((x.shape[0], 2, x.shape[1]//2, x.shape[-1]))
        x_month = torch.mean(x_month, axis = 2)

        x_month,_ = self.rnn6(x_month.permute(1,0,2))

        x_signal = self.pointwise1(x_signal[-10:,:,:].permute(1,0,2)).squeeze(1)
        x_day = self.pointwise2(x_day[-4:,:,:].permute(1,0,2)).squeeze(1)
        x_week = self.pointwise3(x_week[-2:,:,:].permute(1,0,2)).squeeze(1)
        x_month = self.pointwise4(x_month[-1:,:,:].permute(1,0,2)).squeeze(1)

        x_signal = x_signal + x_day +x_week +x_month
        x_signal = self.dropout(x_signal)
        x_signal = self.act(x_signal)
        x_signal = self.linear1(x_signal)
...
class rnn_sxy_2_7_false_3(BasicModule):
    def __init__(self,input_dim=482,drop_rate=0.5):
        super(rnn_sxy_2_7_false_3,self).__init__()
        self.model_name = 'rnn_sxy_2_7_false_3:all_time'
        
        self.rnn0 = nn.LSTM(input_size = 12, hidden_size = 256, num_layers = 3)    
        self.rnn1 = nn.LSTM(input_size = 12, hidden_size = 256, num_layers = 3)    
        self.rnn2 = nn.LSTM(input_size = 12, hidden_size = 256, num_layers = 3)


        self.rnn3 = nn.LSTM(input_size = 12, hidden_size = 256, num_layers = 3)

        self.rnn4 = nn.LSTM(input_size = 12, hidden_size = 256, num_layers = 3)


        self.rnn5 = nn.LSTM(input_size = 12, hidden_size = 256, num_layers = 3)


        self.pointwise0 = nn.Conv1d(in_channels=32, out_channels=1, kernel_size=1)
        self.pointwise1 = nn.Conv1d(in_channels=16, out_channels=1, kernel_size=1)
        self.pointwise2 = nn.Conv1d(in_channels=8, out_channels=1, kernel_size=1)

        self.pointwise3 = nn.Conv1d(in_channels=4, out_channels=1, kernel_size=1)
        self.pointwise4 = nn.Conv1d(in_channels=2, out_channels=1, kernel_size=1)
        self.pointwise5 = nn.Conv1d(in_channels=1, out_channels=1, kernel_size=1)
    ...
    def forward(self,x):
        ...
        x_signal,_ = self.rnn0(x[:,:,:].permute(1,0,2))

        x_hour = x.reshape((x.shape[0], 16, x.shape[1]//16, x.shape[-1]))
        x_hour = torch.mean(x_hour, axis = 2)
        x_hour,_ = self.rnn1(x_hour.permute(1,0,2))
        ...
        x_week = x.reshape((x.shape[0], 4, x.shape[1]//4, x.shape[-1]))
        x_week = torch.mean(x_week, axis = 2)
        x_week,_ = self.rnn3(x_week.permute(1,0,2))

        x_month = x.reshape((x.shape[0], 2, x.shape[1]//2, x.shape[-1]))
        x_month = torch.mean(x_month, axis = 2)
        x_month,_ = self.rnn4(x_month.permute(1,0,2))

        x_all = x.reshape((x.shape[0], 1, x.shape[1]//1, x.shape[-1]))
        x_all = torch.mean(x_all, axis = 2)
        x_all,_ = self.rnn5(x_all.permute(1,0,2))

        x_signal = self.pointwise0(x_signal[-32:,:,:].permute(1,0,2)).squeeze(1)
        x_hour = self.pointwise1(x_hour[-16:,:,:].permute(1,0,2)).squeeze(1)
        x_day = self.pointwise2(x_day[-8:,:,:].permute(1,0,2)).squeeze(1)
        x_week = self.pointwise3(x_week[-4:,:,:].permute(1,0,2)).squeeze(1)
        x_month = self.pointwise4(x_month[-2:,:,:].permute(1,0,2)).squeeze(1)
        x_all = self.pointwise5(x_all[-1:,:,:].permute(1,0,2)).squeeze(1)

        x_signal = x_signal + x_hour +x_day +x_week +x_month +x_all
        x_signal = self.dropout(x_signal)
        x_signal = self.act(x_signal)
        x_signal = self.linear1(x_signal)
...
class rnn_sxy_2_7_false_4(BasicModule):
    def __init__(self,input_dim=482,drop_rate=0.5):
        super(rnn_sxy_2_7_false_4,self).__init__()
        self.model_name = 'rnn_sxy_2_7_false_4:all_time'
        ...
        self.pointwise0 = nn.Conv1d(in_channels=16, out_channels=1, kernel_size=1)
        self.pointwise1 = nn.Conv1d(in_channels=8, out_channels=1, kernel_size=1)
        self.pointwise2 = nn.Conv1d(in_channels=4, out_channels=1, kernel_size=1)
    ...
    def forward(self,x):
        ...
        x_signal = self.pointwise0(x_signal[-16:,:,:].permute(1,0,2)).squeeze(1)
        x_hour = self.pointwise1(x_hour[-8:,:,:].permute(1,0,2)).squeeze(1)
        x_day = self.pointwise2(x_day[-4:,:,:].permute(1,0,2)).squeeze(1)
        x_week = self.pointwise3(x_week[-2:,:,:].permute(1,0,2)).squeeze(1)
        x_month = self.pointwise4(x_month[-1:,:,:].permute(1,0,2)).squeeze(1)
        x_all = self.pointwise5(x_all[-1:,:,:].permute(1,0,2)).squeeze(1)
...
class rnn_sxy_2_7_false_5(BasicModule):
    def __init__(self,input_dim=482,drop_rate=0.5):
        super(rnn_sxy_2_7_false_5,self).__init__()
        self.model_name = 'rnn_sxy_2_7_false_5:all_time'
...
class rnn_sxy_2_35(BasicModule):
    def __init__(self,input_dim=482,hidden_size = 30, num_layers = 1, dropout = 0):
        ...
        self.model_name = 'rnn_sxy_2_35:daily'

        self.rnn1 = nn.LSTM(input_size = 12, hidden_size = 256, num_layers = 3)

        self.rnn2 = nn.LSTM(input_size = 12, hidden_size = 256, num_layers = 3)

        self.pointwise1 = nn.Conv2d(in_channels=8,out_channels=1,kernel_size=1)
        self.pointwise2 = nn.Conv2d(in_channels=32,out_channels=1,kernel_size=1)
        self.act1 = nn.ReLU()
        self.act2 = nn.ReLU()

        self.linear1 = nn.Linear(256,30)
    ...
    def forward(self,x):
        ...
        x_signal_day = x_signal_day[-8:,:,:]
        x_signal_bar = x_signal_bar[-32:,:,:]
